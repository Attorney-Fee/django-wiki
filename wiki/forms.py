...
class UserCreationForm(UserCreationForm):
    ...
    def clean(self):
        cd = self.cleaned_data
        if hasattr(site_settings, 'FORCE_EMAIL_DOMAIN'):
            if not cd['email'].endswith(site_settings.FORCE_EMAIL_DOMAIN):
                raise forms.ValidationError('Sorry, you are not allowed to create an account')
